...
def generate_cost_matrices(predMat,weight,N):
    ...
    costMat.H = predMat.S.T@scipy.linalg.block_diag(np.kron(np.eye(N),weight.Q),weight.P)@predMat.S + np.kron(np.eye(N),weight.R)
    costMat.Hi = np.linalg.inv(costMat.H)
    costMat.F = predMat.S.T@scipy.linalg.block_diag(np.kron(np.eye(N),weight.Q),weight.P)@predMat.P
